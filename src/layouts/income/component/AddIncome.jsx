...
import { apiCreateIncome } from "../../../services/IncomeApi";
import { getSelectCategory } from "../../../services/CategoryApi";

const { Option } = Select;

const AddIncome = ({ visible, onCancel, user, fetchData, handleAlert }) => {
	...
	const handleOk = async (values) => {
		try {
			...
			const data = await apiCreateIncome(values);
			if (data.status === 201) {
				fetchData();
				handleAlert(`Jenis pemasukan ${values.name} berhasil ditambah`);
				onCancel();
			}
		} catch (error) {
